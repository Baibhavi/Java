...
If all parameters are valid, the method needs to calculate the number of buckets and return it.Examples of input/output:
...
public class PaintJob {
    public static void main(String[] args){
        ...
        System.out.println("Enter the width, height, area per bucket and extra buckets painter have respectively :");
        double w = sc.nextDouble();
        double l = sc.nextDouble();
        double apb = sc.nextDouble();
        int b = sc.nextInt();
        System.out.println(getBucketCount(w,l,apb,b));
        System.out.println(getBucketCount(w, l, apb));
        System.out.println(getBucketCount(w*l, apb));
        sc.close();
    }
    public static int getBucketCount(double width,double height,double areaPerBucket,int extraBuckets){
        ...
        double areaCanbePainted = (areaPerBucket*extraBuckets);
        double areaLeftUnpainted = Wallarea-areaCanbePainted;
        if (areaLeftUnpainted<0){
            return 0;
        }
        else{
            int bucketsNeeded = ((int)Math.ceil(areaLeftUnpainted/areaPerBucket));
